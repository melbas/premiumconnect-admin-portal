...
// Import rate limiter for auth (100 req/min)
import { apiRateLimiter, getClientIP, createRateLimitHeaders } from '../_shared/rate-limiter.ts';
...
serve(async (req) => {
  ...
  // Rate limiting - 100 requests per minute per IP for RADIUS auth
  const clientIP = getClientIP(req);
  const isLimited = apiRateLimiter.isLimited(clientIP);
  const rateLimitInfo = apiRateLimiter.getRateLimitInfo(clientIP);
  
  if (isLimited) {
    console.log(`Rate limit exceeded for RADIUS auth from IP: ${clientIP}`);
